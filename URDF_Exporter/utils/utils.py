# -*- coding: utf-8 -*-
"""Shared CAD and URDF utility functions."""
import shutil
try:
    import adsk
    import adsk.core
...
def valid_name(name):
    """
    한글, 특수문자, 공백 등을 안전한 영문(comp_)과 언더바(_)로 치환합니다.
    예: '구성요소14_1' -> 'comp_14_1'
    """
    res = re.sub(r'[^a-zA-Z0-9_]', 'comp_', name)
    res = re.sub(r'(comp_)+', 'comp_', res)
    res = re.sub(r'_+', '_', res).strip('_')
    
    # 완전히 숫자가 없는 한글 이름일 경우를 대비해 해시 추가
    if not res or res == 'comp':
        res = "part_" + hashlib.md5(name.encode('utf-8')).hexdigest()[:6]
    return res
